from datetime import datetime
   ...
   def __init__(self, member_id, name):
    ...
    def __str__(self):
    entry = self.entry_time.strftime('%Y-%m-%d %H:%M:%S') if self.entry_time else 'Notchecked in'
    exit = self.exit_time.strftime('%Y-%m-%d %H:%M:%S') if self.exit_time else 'Not checkedout'
    return f"Member ID: {self.member_id}, Name: {self.name}, Entry Time: {entry}, Exit Time:
    {exit}"
    class Book:
    ...
    return f"Book ID: {self.book_id}, Title: {self.title}, Author: {self.author}, Borrowed By:
    {borrowed_by}"
    class Library:
    def __init__(self):
    self.members = {}
    ...
    print(f"Book '{self.books[book_id].title}' is already borrowed by
    {self.books[book_id].borrowed_by.name}.")
    else:
